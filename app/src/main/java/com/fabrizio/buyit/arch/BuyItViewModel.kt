...
class BuyItViewModel():ViewModel() {

    private lateinit var repository : BuyItRepository

    val itemEntitiesLiveData = MutableLiveData<List<ItemEntity>>()
    val itemWithCategoryEntitiesLiveData = MutableLiveData<List<ItemWithCategoryEnity>>()
    ...
    fun init(appDataBase: AppDatabase) {
        repository = BuyItRepository(appDataBase)

        // Initialize our Flow connectivity to the DB for ItemEntities and CatagoryEntities
    ...
    private fun updateHomeViewState(items: List<ItemWithCategoryEnity>) {
